...
export default function CarritoPage() {
  ...
  const subtotal = itemsSeleccionados.reduce((s, i) => s + i.precio * i.cantidad, 0)
  ...
  function toggleSeleccion(id: string) {
    ...
    setResultado(null); setPdfUrl(null)
  }
  ...
  function handleWhatsApp() {
    ...
    const msg = encodeURIComponent(
      `¡Hola! Soy ${nombre} y acabo de generar una cotización en D'kore.\n\n` +
      `📋 *Cotización N° ${resultado.numeroCotizacion}*\n` +
      `📅 Válida hasta: ${resultado.fechaValidez}\n` +
      `💰 Total: $${resultado.total.toFixed(2)} USD\n\n` +
      `Adjunto el PDF con el detalle completo de mi pedido. ¿Podrían confirmarme disponibilidad y tiempo de entrega? ¡Gracias!`
    )
  ...
  if (items.length === 0) {
    return (
      <main style={{ minHeight: '100vh', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '20px', paddingTop: '80px' }}>
        <p style={{ color: '#6b7280', fontSize: '1rem' }}>Tu carrito está vacío</p>
        <Link href="/catalogo">
          <button style={{ padding: '12px 28px', borderRadius: '8px', border: 'none', background: '#C5A059', color: 'black', cursor: 'pointer', fontWeight: '700' }}>Ver catálogo</button>
        </Link>
      </main>
    )
  }
    ...
    <main style={{ minHeight: '100vh', background: '#000', paddingTop: '90px', paddingBottom: '60px' }}>
      <div className="carrito-grid" style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 16px' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {item.configuracion && (
                        <p style={{ color: '#9ca3af', fontSize: '0.72rem', margin: '0 0 2px' }}>
                          {item.configuracion.alto}m × {item.configuracion.profundidad}m × {item.configuracion.ancho}m — {item.configuracion.tipoMelamina}
                        </p>
                      )}
        ...
        <div className="carrito-sidebar" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div>
                  ...
                  {errores.cedula
                    ...
                    : cliente.cedula.length === 10 && validarCedula(cliente.cedula)
                      ? <p style={{ color: '#22c55e', fontSize: '0.72rem', marginTop: '4px' }}>✓ Cédula válida</p>
                      : null
                  }
                </div>
                <div>
          ...
          {pdfUrl && (
            <div style={{ background: '#0d0d0d', borderRadius: '14px', border: '1px solid #2a2a2a', overflow: 'hidden' }}>
              <div style={{ padding: '14px 18px', borderBottom: '1px solid #1f1f1f', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <p style={{ color: 'white', fontWeight: '700', fontSize: '0.875rem', margin: 0 }}>Cotización generada</p>
                  <p style={{ color: '#6b7280', fontSize: '0.75rem', margin: '2px 0 0' }}>N° {resultado?.numeroCotizacion} · Válida hasta {resultado?.fechaValidez}</p>
                </div>
              ...
              <iframe ref={pdfRef} src={pdfUrl} style={{ width: '100%', height: '480px', border: 'none', background: 'white' }} title="Preview cotización" />
            </div>
          )}
          ...
          <div style={{ background: '#0d0d0d', borderRadius: '14px', border: '1px solid #1f1f1f', padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <h2 style={{ color: 'white', fontSize: '1rem', fontWeight: '700', margin: 0 }}>Resumen</h2>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
              {itemsSeleccionados.length === 0 ? (
                <p style={{ color: '#4b5563', fontSize: '0.82rem' }}>Ningún producto seleccionado</p>
              ) : (
                itemsSeleccionados.map(item => (
                  <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
                    <span style={{ color: '#9ca3af', fontSize: '0.8rem', flex: 1 }}>{item.nombre} {item.color ? `(${item.color})` : ''} ×{item.cantidad}</span>
                    <span style={{ color: 'white', fontSize: '0.8rem', flexShrink: 0 }}>{item.precio > 0 ? `$${(item.precio * item.cantidad).toFixed(2)}` : 'A cotizar'}</span>
                  </div>
                ))
              )}
            </div>
            ...
            <div style={{ borderTop: '1px solid #1f1f1f', paddingTop: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                <span style={{ color: '#9ca3af', fontSize: '0.85rem' }}>Subtotal</span>
                <span style={{ color: 'white', fontSize: '0.85rem' }}>${subtotal.toFixed(2)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                <span style={{ color: '#9ca3af', fontSize: '0.85rem' }}>Envío</span>
                <span style={{ color: envioGratis ? '#4ade80' : '#6b7280', fontSize: '0.85rem' }}>{envioGratis ? 'Gratis 🎉' : 'A coordinar'}</span>
              </div>
              {!envioGratis && subtotal > 0 && (
                <p style={{ color: '#4b5563', fontSize: '0.72rem', margin: '0 0 10px', textAlign: 'right' }}>
                  Agrega ${(ENVIO_GRATIS_DESDE - subtotal).toFixed(2)} más para envío gratis en Cuenca
                </p>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #1f1f1f', paddingTop: '12px' }}>
                <span style={{ color: 'white', fontWeight: '700' }}>Total</span>
                <span style={{ color: '#C5A059', fontWeight: '700', fontSize: '1.1rem' }}>${subtotal.toFixed(2)}</span>
              </div>
            </div>
            ...
            {!resultado ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <button onClick={handleDescargar} style={{ width: '100%', padding: '13px', borderRadius: '10px', border: 'none', background: '#C5A059', color: 'black', cursor: 'pointer', fontWeight: '700', fontSize: '0.9rem' }}>Descargar cotización</button>
            ...
            <p style={{ color: '#4b5563', fontSize: '0.72rem', textAlign: 'center', margin: 0 }}>La cotización tiene validez de 15 días desde su emisión</p>
          </div>
        </div>
      </div>
      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          ...
          /* En móvil el formulario va a 1 columna */
          .form-grid {
            grid-template-columns: 1fr;
          }
          .form-full {
